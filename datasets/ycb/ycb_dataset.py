...
class YCB_Dataset:
    ...
    def get_item(self, item_name):
        try:
            ...
            edge_img = np.transpose(rgb, (1, 2, 0))
            edge_img = edge_img.astype(np.uint8)
            edge_img = cv2.cvtColor(edge_img, cv2.COLOR_BGR2GRAY)
            edge = (cv2.Canny(edge_img, 60, 60 * 1.5)).astype(np.float32)[:, :, np.newaxis]
            ...
            return torch.from_numpy(rgb.astype(np.float32)), \
                ...
                torch.LongTensor(labels.astype(np.int32)), \
                torch.from_numpy(kp3ds.astype(np.float32)), \
                torch.from_numpy(ctr3ds.astype(np.float32)), torch.from_numpy(edge.astype(np.float32)), \
