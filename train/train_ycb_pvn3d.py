...
from lib import KP_SaMfENet
from lib.loss import OFLoss, FocalLoss, Edge_Loss
...
def model_fn_decorator(
        criterion, criterion_of, edge_loss, test=False
):
    ...
    def model_fn(
        ...
        with torch.set_grad_enabled(not is_eval):
            ...
            rgb, pcld, cld_rgb_nrm, choose, kp_targ_ofst, ctr_targ_ofst, \
                cls_ids, rts, labels, kp_3ds, ctr_3ds, edge_targ, edge_weight = cu_dt

            pred_kp_of, pred_rgbd_seg, pred_ctr_of, pred_edge = model(
            ...
            loss_edge_pred = edge_loss(edge_targ, pred_edge, edge_weight).sum()
            w = [2.0, 1.0, 1.0, 0.5]
            loss = loss_rgbd_seg * w[0] + loss_kp_of * w[1] + \
                   loss_ctr_of * w[2] + loss_edge_pred * w[3]
...
if __name__ == "__main__":
    ...
    if not args.eval_net:
        ...
        train_loader = torch.utils.data.DataLoader(
            ...
            num_workers=10, worker_init_fn=worker_init_fn
        )
    ...
    model = KP_SaMfENet(
        num_classes=22, num_points=config.n_sample_points).cuda()
